...
export default function LoginClient(props: { next?: string }) {
  ...
  const nextParam = String(props.next || "").trim();
    ...
    (async () => {
      ...
      if (data?.user) {
        try {
          ...
          // show login UI if something fails
        }
      }
    })();
  ...
  return (
    <div style={{ maxWidth: 440, margin: "48px auto", padding: 16 }}>
      ...
      <p style={{ opacity: 0.8, marginBottom: 24 }}>Sign in to complete onboarding and open your company.</p>
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        {msg ? (
          <div style={{ marginTop: 8, padding: 12, borderRadius: 12, background: "#f7f7f7" }}>{msg}</div>
        ) : null}
      </form>
    </div>
  );
