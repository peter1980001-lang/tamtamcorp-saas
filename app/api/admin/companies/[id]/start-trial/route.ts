...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const days = Number.isFinite(Number(body?.days)) ? Math.max(1, Math.min(60, Number(body.days))) : 14;
  ...
  if (!plan?.is_active) return NextResponse.json({ error: "plan_not_found_or_inactive" }, { status: 400 });
