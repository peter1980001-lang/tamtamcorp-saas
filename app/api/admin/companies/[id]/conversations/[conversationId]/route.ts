...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string; conversationId: string }> }) {
  ...
  if (String((conv as any).company_id) !== company_id) return NextResponse.json({ error: "company_mismatch" }, { status: 403 });

  const { data: lead, error: lErr } = await supabaseServer
    .from("company_leads")
    .select("id, company_id, conversation_id, lead_state, status, score_total, score_band, intent_score, name, email, phone, qualification_json, consents_json, last_touch_at, created_at, updated_at")
