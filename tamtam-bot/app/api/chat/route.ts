...
type CompanyChatConfig = {
  ...
  // optional per-company overrides (will be capped by plan entitlements)
  rate_limits?: {
    per_minute?: number;
    per_day?: number;
  };
...
function normalizeConfig(raw: any): CompanyChatConfig {
  const defaults: CompanyChatConfig = {
    ...
    rate_limits: {
      per_minute: 10, // default 10/min
      per_day: 1000,  // default 1000/day
    },
  };
...
async function loadCompanyEntitlements(company_id: string): Promise<{
  ...
  // default: locked down (no chat) if no billing row
  ...
  // If we have a mapped plan_key or price_id, load entitlements
...
export async function POST(req: Request) {
  ...
  // 3) Conversation must exist and belong to company (prevents cross-tenant leaks)
  ...
  // 4) Billing gate (Plan B: Entitlements) - BEFORE any paid compute (embeddings/openai)
  ...
  // 5) Load per-company config
  const cfg = await loadCompanyConfig(company_id);

  // 6) Apply plan caps to rate limits (hard enforcement)
  const effectiveLimits = capRateLimits(cfg, billing.entitlements);

  // 7) Store user message (no ghost replies)
  ...
  // 8) Retrieve knowledge chunks (RPC expects embedding vector!)
  let chunks: any[] = [];

  // 8a) create query embedding
  ...
  // 9) If knowledge_only and no chunks => strict fallback
  ...
  // 10) ATOMIC RATE LIMIT ENFORCEMENT (BEFORE OpenAI CHAT CALL)
